...
namespace Hestia.ConsoleRunner
{
    public class Runner
    {
        ...
        public void BuildFromConfig(RunnerConfig config) =>
            ConfigAsBuilder(config)
                ...
                .Apply(x =>
                {
                    var canParse = Enum.Parse(typeof(GitStatGranularity), config.StatGranularity, true) is GitStatGranularity;
                    if (!canParse)
                    {
                    ...
                    var granularity = (GitStatGranularity)Enum.Parse(typeof(GitStatGranularity), config.StatGranularity, true);

                    _log.Information($"Enriching snapshot with git stats with {granularity.ToString()} granularity");
                    return _statsEnricher.EnrichWithGitStats(x, granularity);
                })
                .Apply(snapshot =>
                {
                    Log.Information("Saving enriched snapshot to database");
                    _persistence.InsertSnapshot(snapshot);
                    Log.Information("Committed to DB successfully.");
